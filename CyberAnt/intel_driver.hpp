...
namespace intel_driver
{
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments)
	{
		...
		if (ntdll == 0) {
			std::cout << xorstr_("[-] Failed to load ntdll.dll") << std::endl; //never should happens
			return false;
		}

		const auto NtQueryInformationAtom = reinterpret_cast<void*>(GetProcAddress(ntdll, xorstr_("NtQueryInformationAtom")));
		if (!NtQueryInformationAtom)
		{
			std::cout << xorstr_("[-] Failed to get export ntdll.NtQueryInformationAtom") << std::endl;
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		const uint64_t kernel_NtQueryInformationAtom = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")), xorstr_("NtQueryInformationAtom"));
		if (!kernel_NtQueryInformationAtom)
		{
			std::cout << xorstr_("[-] Failed to get export ntoskrnl.NtQueryInformationAtom") << std::endl;
			return false;
		}
