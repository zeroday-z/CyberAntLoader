...
namespace lw_http_tools
{
	...
	std::string		url_encode(std::string s_input);
	std::string		url_decode(std::string s_input);
}
...
public:
	...
	bool	get(std::string sURL, std::string& s_reply);
	//bool	get(std::wstring s_url, std::vector<BYTE>& btReply);

	bool	post(std::string sURL, const char* sReply, c_lw_httpd& PostData);
