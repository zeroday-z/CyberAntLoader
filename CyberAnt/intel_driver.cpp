...
{
	LPCWSTR loc = wxorstr_(L"\\\\.\\Nal");
	const HANDLE file_handle = CreateFileW(loc, FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
	ZeroMemory(&loc, sizeof(loc));
	loc = wxorstr_(L"xxxxxxx");
...
HANDLE intel_driver::Load()
{
	...
	return CreateFileW(loc, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
}
...
void intel_driver::Unload(HANDLE device_handle)
{
	...
	{
		std::cout << xorstr_("[!] Error dumping shit inside the disk") << std::endl;
	}
	else {
	}
	file_ofstream.close();
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	static uint64_t kernel_ExAllocatePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")), xorstr_("ExAllocatePoolWithTag"));

	if (!kernel_ExAllocatePool)
	{
		std::cout << xorstr_("[!] Failed to find ExAllocatePool") << std::endl;
		return 0;
	}
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	...
	static uint64_t kernel_ExFreePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")), xorstr_("ExFreePool"));
...
bool intel_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	...
	while (status == nt::STATUS_INFO_LENGTH_MISMATCH2)
	{
	...
	if (!ReadMemory(device_handle, object + 0x8, &device_object, sizeof(device_object)) || !device_object) {
		std::cout << xorstr_("[!] Failed to find device_object") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, device_object + 0x8, &driver_object, sizeof(driver_object)) || !driver_object) {
		std::cout << xorstr_("[!] Failed to find driver_object") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_object + 0x28, &driver_section, sizeof(driver_section)) || !driver_section) {
		std::cout << xorstr_("[!] Failed to find driver_section") << std::endl;
		return false;
	}
	...
	if (!ReadMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name)) || us_driver_base_dll_name.Length == 0) {
		std::cout << xorstr_("[!] Failed to find driver name") << std::endl;
		return false;
	}
	...
	if (!WriteMemory(device_handle, driver_section + 0x58, &us_driver_base_dll_name, sizeof(us_driver_base_dll_name))) {
		std::cout << xorstr_("[!] Failed to write driver name length") << std::endl;
		return false;
	}
...
bool intel_driver::ExAcquireResourceExclusiveLite(HANDLE device_handle, PVOID Resource, BOOLEAN wait)
{
	...
	if (!kernel_ExAcquireResourceExclusiveLite) {
		std::cout << xorstr_("[!] Failed to find ExAcquireResourceExclusiveLite") << std::endl;
		return 0;
	}
...
bool intel_driver::ExReleaseResourceLite(HANDLE device_handle, PVOID Resource)
{
	...
	if (!kernel_ExReleaseResourceLite) {
		std::cout << xorstr_("[!] Failed to find ExReleaseResourceLite") << std::endl;
		return false;
	}
...
BOOLEAN intel_driver::RtlDeleteElementGenericTableAvl(HANDLE device_handle, PVOID Table, PVOID Buffer)
{
	...
	if (!kernel_RtlDeleteElementGenericTableAvl) {
		std::cout << xorstr_("[!] Failed to find RtlDeleteElementGenericTableAvl") << std::endl;
		return false;
	}
...
bool intel_driver::ClearPiDDBCacheTable(HANDLE device_handle) { //PiDDBCacheTable added on LoadDriver
	...
	uint64_t ntoskrnl = utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe"));
	...
	if (PiDDBLockPtr == NULL || PiDDBCacheTablePtr == NULL) {
		std::cout << xorstr_("[-] Warning no PiDDBCacheTable Found") << std::endl;
		return false;
	}
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, PiDDBLock, true)) {
		std::cout << xorstr_("[-] Can't lock PiDDBCacheTable") << std::endl;
		return false;
	}
	...
	if (pFoundEntry == nullptr) {
		std::cout << xorstr_("[-] Not found in cache") << std::endl;
	...
	if (!RtlDeleteElementGenericTableAvl(device_handle, PiDDBCacheTable, pFoundEntry)) {
		std::cout << xorstr_("[-] Can't delete from PiDDBCacheTable") << std::endl;
		ExReleaseResourceLite(device_handle, PiDDBLock);
		return false;
	}
...
uintptr_t intel_driver::FindPatternAtKernel(HANDLE device_handle, uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask) {
	if (!dwAddress) {
		std::cout << xorstr_("[-] No module address to find pattern") << std::endl;
		return 0;
	}

	if (dwLen > 1024 * 1024 * 1024) { //if read is > 1GB
		std::cout << xorstr_("[-] Can't find pattern, Too big section") << std::endl;
		return 0;
	}
	...
	if (result <= 0) {
		std::cout << xorstr_("[-] Can't find pattern") << std::endl;
		delete[] sectionData;
		return 0;
	}
...
bool intel_driver::ClearKernelHashBucketList(HANDLE device_handle) {
	...
	wdname = L"\\" + wdname;
	uint64_t ci = utils::GetKernelModuleAddress(xorstr_("ci.dll"));
	...
	if (!sig) {
		std::cout << xorstr_("[-] Can't Find g_KernelHashBucketList") << std::endl;
		return false;
	}
	auto sig2 = FindPatternAtKernel(device_handle, (uintptr_t)sig - 50, 50, PUCHAR("\x48\x8D\x0D"), (char*)"xxx");
	if (!sig2) {
		std::cout << xorstr_("[-] Can't Find g_HashCacheLock") << std::endl;
		return false;
	}
	...
	{
		std::cout << xorstr_("[-] Can't Find g_HashCache relative address") << std::endl;
		return false;
	}
	...
	if (!ExAcquireResourceExclusiveLite(device_handle, g_HashCacheLock, true)) {
		std::cout << xorstr_("[-] Can't lock g_HashCacheLock") << std::endl;
		return false;
	}
